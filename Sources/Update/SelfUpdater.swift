...
enum SelfUpdater {
    /// Our Apple Developer Team ID — the downloaded update must be signed by this team or it's rejected.
    static let teamID = "834D8P4J32"
    ...
    static func installUpdate(from dmgURL: URL,
                              teamID: String = SelfUpdater.teamID,
                              status: @MainActor @escaping (String) -> Void) async throws {
        let fm = FileManager.default
        let work = fm.temporaryDirectory.appendingPathComponent("ibe-update-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func verify(app: String, teamID: String) throws {
        // 1. The signature is intact and covers the whole bundle (incl. any nested code).
